...
export default function Home() {
  ...
  const [websiteBasics, setWebsiteBasics] = useState<WebsiteBasics | null>(
    null,
  );
  ...
  useEffect(() => {
    async function loadBusinessInfo() {
      const { data } = await supabase
        .from("business_info")
        .select(
          "address, phone, email, opening_hours, facebook, instagram, tiktok, order_pickup, order_delivery",
        )
  ...
  useEffect(() => {
    ...
    initHomeAnimations(disableMotion);
  }, [isAppReady, disableMotion]);
  ...
  const menuPdfHref = menuPdfUrl ?? undefined;
  const hasMenuPdf = Boolean(menuPdfHref);
  const hasOrderLinks = Boolean(
    businessInfo?.order_delivery || businessInfo?.order_pickup,
  );
  const thumbImages = [
      ...
      {!isAppReady && (
        ...
        >
          ...
          >
            <style>
              {
                ".bowl{transform-origin:50% 60%;animation:b 2.8s ease-in-out infinite}.steam path{animation:s 1.8s ease-in-out infinite}.steam path:nth-child(2){animation-delay:.3s}.steam path:nth-child(3){animation-delay:.6s}@keyframes s{0%{opacity:0;transform:translateY(4px)}20%{opacity:1}100%{opacity:0;transform:translateY(-6px)}}@keyframes b{0%{transform:rotate(0)}25%{transform:rotate(-1deg)}50%{transform:rotate(0)}75%{transform:rotate(1deg)}100%{transform:rotate(0)}}"
              }
            </style>
            <g className="bowl">
      ...
      <main className="page-bg w-full overflow-x-hidden ">
        ...
        <header className="fixed top-0 left-0 w-full z-50 ">
          <div className="  p-3 flex items-center justify-between ">
            ...
            <div className="flex items-center gap-6">
              ...
              <a
                className="bg-white text-brand-primary px-5 py-2 rounded-full text-sm font-semibold tracking-wide hover:bg-surface transition shadow-xl hidden md:block"
                href="/login"
                target="_blank"
              >
                Admin Demo
        ...
        <section className="section-hero relative h-screen w-full overflow-hidden">
          ...
          <div className="hero-content relative z-20 h-full flex flex-col items-center justify-center text-center text-white px-6 m">
            <div className="relative z-20 h-full flex flex-col items-center justify-center text-center text-white px-0 sm:px-6">
              <h1
                className="hero-title text-4xl lg:text-8xl font-light mb-6  tracking-tight "
              ...
              <button
                onClick={() => scrollToSection(".section-about", disableMotion)}
                className="hero-cta inline-flex items-center justify-center px-6 py-3 rounded-full  border border-white text-white hover:bg-white hover:text-black transition-colors "
              >
                {t("hero_button_text")}{" "}
                <HiOutlineArrowDown className="ml-3 text-md" />
              </button>
            </div>
          </div>
        </section>
        ...
        <section className=" section-about min-h-screen flex items-center px-5 md:px-10 py-[10vh] ">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 w-full max-w-6xl mx-auto items-center">
            <div className="about-left">
              <h2
                className="text-4xl md:text-6xl font-normal text-primary-brand mb-6 tracking-tight moving-text "
            ...
            <div className="about-card bg-brand-primary text-white shadow-lg rounded-2xl p-5 pb-10 pt-10 md:pt-12 md:pb-12 md:p-12">
              ...
              <p
                className="text-md md:text-lg mb-6 leading-relaxed font-light"
              ...
              {hasMenuPdf && (
                ...
                >
                  {isSV ? "Visa vår Meny" : "View our Menu"}{" "}
                  <LuUtensilsCrossed className="ml-3 text-lg" />
                </a>
              )}
            </div>
          </div>
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {businessInfo?.order_delivery && (
                      <a
                        href={`${businessInfo.order_delivery}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-full inline-flex items-center justify-center px-6 py-3 rounded-full bg-white text-brand-primary font-semibold tracking-wide hover:bg-surface transition "
                      >
                        {isSV ? "Leverans" : "Order Delivery"}{" "}
                        <RiEBike2Line className="text-lg ml-3" />
                      </a>
                    )}

                    {businessInfo?.order_pickup && (
                      <a
                        href={`${businessInfo.order_pickup}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="w-full inline-flex items-center justify-center px-6 py-3 rounded-full bg-white text-brand-primary font-semibold tracking-wide hover:bg-surface transition "
                      >
                        {isSV ? "Avhämtning" : "Order Pickup"}{" "}
                        <RiShoppingBag3Line className="ml-3 text-lg" />
                      </a>
                    )}
                  </div>
                </>
            ...
            <div className="map-container order-1 md:order-2">
              ...
              <div className="map-frame w-full h-80 md:h-96 rounded-xl shadow-xl border border-[#E3E1DA] hover:!opacity-100 transition-opacity duration-600 overflow-hidden">
                {websiteBasics?.google_maps && (
                  <div
                    dangerouslySetInnerHTML={{
                      __html: websiteBasics.google_maps,
                    }}
                  />
                )}
              </div>
        ...
        <section className="section-contact min-h-screen bg-brand-primary text-white px-10 py-24 flex items-center justify-center">
          <div className="contact-inner text-center w-full ">
            <h2
              className="text-4xl md:text-5xl font-light mb-10 max-w-xl mx-auto tracking-tight moving-text "
            ...
            <footer className="border-t border-white/20 pt-6 max-w-3xl mx-auto">
              <p className="text-sm opacity-70 mb-10">
                © {new Date().getFullYear()} TheGreek — React & Next.js Website
                Template — View project on{" "}
                <a
              ...
              <a
                ...
                className="bg-white text-brand-primary px-5 py-2 rounded-full text-sm font-semibold tracking-wide hover:bg-surface transition shadow-xl inline-flex items-center justify-center"
              >
                {isSV ? "Till Toppen" : "Back to Top"}{" "}
                <HiOutlineArrowUp className="ml-3" />
              </a>
            </footer>
          </div>
        </section>
        ...
        {lightboxImage && (
          ...
          >
            {/* Close button */}
            <button
              className=" absolute top-2 md:top-4 right-2 md:right-4 w-10 h-10 rounded-full bg-white text-black text-xl font-bold flex items-center justify-center hover:scale-110 transition z-[110] "
              onClick={(e) => {
                e.stopPropagation();
            ...
            <img
              src={lightboxImage}
              className=" max-w-full max-h-full rounded-lg md:rounded-2xl shadow-2xl animate-[fadeIn_0.4s_ease-out] "
